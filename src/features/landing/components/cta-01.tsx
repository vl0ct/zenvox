...
export function Cta01({ title, description, cta }: Readonly<Cta01Props>) {
  return (
    <div className="relative w-full overflow-hidden rounded-2xl shadow-sm bg-muted py-16 sm:py-20">
      <div className="mx-auto flex max-w-lg flex-col items-center justify-center px-6">
        <div className="flex flex-col items-center space-y-4 self-center">
          <div className="space-y-2">
            {title && (
              <h2 className="text-center text-3xl text-foreground/80 font-serif font-normal tracking-tight md:text-4xl md:max-w-200">
          ...
          {cta && <Cta className="mt-4 border-none shadow-sm bg-accent-foreground/50 hover:bg-accent-foreground/70" cta={cta} />}
        </div>
      </div>
    </div>
  );
